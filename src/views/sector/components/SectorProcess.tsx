...
import { SectorData, SectorProcessStep } from '@/types/sector-d-t';
import { ServiceDetailData, ServiceProcessStep } from '@/types/service-d-t';

type ProcessData = SectorData | ServiceDetailData;
...
const SectorProcess = ({ data, title, subtitle = "Working Process" }: SectorProcessProps) => {
    const isSector = 'overview' in data;
    
    // Extract process steps based on type
    let processSteps: (SectorProcessStep | ServiceProcessStep)[] = [];
    
    if (isSector) {
        const sector = data as SectorData;
        processSteps = sector.processSteps || [];
    } else {
        const service = data as ServiceDetailData;
        processSteps = service.processSteps || [];
    }

    const displayTitle = title || `How we deliver ${data.title}`;
                ...
                <div className="row">
                    <div className="col-lg-12">
                        <div className="pp-service-details-process-heading text-center pb-100 tp_fade_anim" data-delay=".3">
                            <span className="pp-service-details-process-subtitle">
                                <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 10 10" fill="none">
                                    <circle cx="5" cy="5" r="5" fill="currentColor" />
                                </svg> {subtitle}
                            </span>
                            <h3 className="pp-service-details-process-title">
                ...
                <div className="row">
                    <div className="col-lg-12">
                        <div className="pp-service-details-process-bottom text-center">
                            <span>Don't hesitate collaborate with expertise- <Link href="/contact-us">
