...
interface ServiceDetailsTwoProcessProps {
    service: ServiceDetailData;
}

const ServiceDetailsTwoProcess = ({ service }: ServiceDetailsTwoProcessProps) => {
    const processSteps = service.processSteps ?? serviceDetailsData[0].processSteps ?? [
        { number: "01", title: "Research And Analysis", description: "Conduct user research (interviews, surveys, analytics)." },
        { number: "02", title: "Design And Prototyping", description: "Transform wireframes into high-fidelity UI designs." },
        { number: "03", title: "Testing And Iteration", description: "Conduct usability testing to gather user feedback." },
        { number: "04", title: "Prepare for Delivery", description: "Track performance using analytics and user feedback." },
    ];
                ...
                <div className="row">
                    <div className="col-lg-12">
                        <div className="pp-service-details-process-heading text-center pb-100 tp_fade_anim" data-delay=".3">
                            ...
                            <h3 className="pp-service-details-process-title">How we deliver <br /> {service.title}</h3>
                        </div>
                    </div>
                </div>
                        ...
                        {processSteps.map((step, index) => (
                            <div className="col-lg-3 col-sm-6" key={index}>
                                <div className="pp-service-details-process-item text-center mb-30">
                                    ...
                                    <h4>{step.title.includes(' & ') ? (
                                            <>
                                                {step.title.split(' & ')[0]} <br />
                                                {step.title.split(' & ')[1]}
                                            </>
                                        ) : step.title.includes(' for ') ? (
                                            <>
                                                {step.title.split(' for ')[0]} <br />
                                                for {step.title.split(' for ')[1]}
                                            </>
                                        ) : (
                                            step.title
                                        )}</h4>
