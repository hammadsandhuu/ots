...
interface PortfolioColFourProps {
    items?: PortfolioItem[];
}

const PortfolioColFour = ({ items }: PortfolioColFourProps) => {
    const displayItems = items ?? portfolioData.slice(33, 41);

    const renderPortfolioItems = (data: PortfolioItem[]) => {
            ...
            <div className="col-xl-3 col-lg-4 col-md-6" key={item.id}>
                <div className="tp-portfolio-inner-item mb-65">
                    ...
                    <div className="tp-portfolio-inner-content">
                        ...
                        <span>{item.category ?? item.categories?.join(", ")}</span>
                    </div>
                </div>
            </div>
        ));
                ...
                <div className="tp-portfolio-tab-content-wrap">
                    <div className="tab-content" id="nav-tabContent">
                        <div className="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab" tabIndex={0}>
                            <div className="row">
                                {renderPortfolioItems(displayItems as PortfolioItem[])}
                            </div>
                        </div>
